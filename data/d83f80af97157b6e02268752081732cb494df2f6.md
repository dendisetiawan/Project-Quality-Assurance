...
- generic [ref=e3]:
  ...
  - generic [ref=e5]:
    - generic [ref=e9]:
      - generic [ref=e10]:
        - textbox "Username" [ref=e11]: process.env.SAUCE_USERNAME!
        - img [ref=e12]
      - generic [ref=e14]:
        - textbox "Password" [ref=e15]: process.env.BASE_URL!
        - img [ref=e16]
...
```ts
  ...
  2 | 
  3 | test('User can login successfully', async ({ loginPage }) => {
  4 |     await loginPage.navigateTo('https://www.saucedemo.com/');
  5 |     await loginPage.login('process.env.SAUCE_USERNAME!', 'process.env.BASE_URL!');
  6 |     
  7 |     // Verifikasi (Gunakan fixture page bawaan Playwright)
> 8 |     await expect(loginPage.page).toHaveURL(/inventory.html/);
    |                                  ^ Error: expect(page).toHaveURL(expected) failed
  9 | });
```